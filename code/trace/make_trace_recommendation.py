...
class gen_trace_data:
        ...
        for rowidx, row in self.df_trace.iterrows():
            ...
            if row["publish"] == 1:
                ...
                if coin <= self.near_ratio:
                    ...
                    #print("Choose from : ", adjlist)
                    # rank the adjlist by Influencial Ranking Algorithm
                    adjlist = sorted(adjlist, key=self.InfluencialRanking, reverse=True)

                    while adjlist:
                        # chose the most influencial user
                        adj_idx = adjlist[0]
                        #print("Chose : ", adj_idx)
    ...
    def InfluencialRanking(self, uid):
        # get the number of followers of the user
        followers = list(self.G.neighbors(uid))
        #print(followers)
        if len(followers) == 0:
            return 0

        followP = len(followers)
        sumP = 0
        for follower in followers:
            sumP += len(list(self.G.neighbors(follower)))

        return followP/math.sqrt(sumP) if sumP != 0 else sys.maxsize
