...
class Build_network:
    ...
    def myNetwork(self):
        ...
        '''对具体的网卡指定对应的ip'''
        for level_1_host_id in range(self.level_1_host_number):
            self.level_1_host[level_1_host_id].cmdPrint('ifconfig a%s-eth%s 0'%(str(level_1_host_id), str(0))) ## eth0和对应的switch相连
            self.level_1_host[level_1_host_id].cmdPrint('ifconfig a%s-eth%s %s.%s/24'%(str(level_1_host_id), str(1), str(switch_gw_pre3), str(100+level_1_host_id))) ## 连接外网

        for level_2_host_id in range(self.level_2_host_number):
            self.level_2_host[level_2_host_id].cmdPrint('ifconfig b%s-eth%s 0'%(str(level_2_host_id), str(self.level_1_host_number))) ## eth0--eth{level_1_host_number-1}和level_1的节点相连，下一个和switch相连。
            self.level_2_host[level_2_host_id].cmdPrint('ifconfig b%s-eth%s %s.%s/24'%(str(level_2_host_id), str(self.level_1_host_number+1), str(switch_gw_pre3), str(150+level_2_host_id))) ## 连接外网

        for level_3_host_id in range(self.level_3_host_number):
            self.level_3_host[level_3_host_id].cmdPrint('ifconfig c%s-eth%s %s.%s/24'%(str(level_3_host_id), str(self.level_2_host_number), str(switch_gw_pre3), str(200+level_3_host_id))) ## 连接外网
        ...
        for switch_id in range(self.level_1_host_number, self.level_1_host_number+self.level_2_host_number):
            for eth_id in range(1, self.level_3_host_number+2): # 和所有level_3_host相连，并连向指定level_2的host和外网
                self.switch[switch_id].attach('sw%s-eth%s'%(str(switch_id), str(eth_id)))
        for eth_id in range(1, self.level_1_host_number+self.level_2_host_number+self.level_3_host_number+1): ## 所有点和外网相连的switch
